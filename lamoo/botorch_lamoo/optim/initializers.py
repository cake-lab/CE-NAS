    ...
    while factor < max_factor:
        with warnings.catch_warnings(record=True) as ws:
            ...
            if inequality_constraints is None and equality_constraints is None:
                ...
                if not kwargs['lamoo_boundary'] or len(kwargs['lamoo_boundary']) == 0:
                    if effective_dim <= SobolEngine.MAXDIM:
                        X_rnd = draw_sobol_samples(bounds=bounds_cpu, n=n, q=q, seed=seed)
                        # print('x_rnd is', X_rnd, X_rnd[0], X_rnd[0][0])
                        # print('n and q are', n, q)
                    else:
                    ...
                    sobol = SobolEngine(dimension=kwargs['problem'].dim, scramble=True, seed=seed)
                    selected_cands = np.zeros((1, kwargs['problem'].dim))
                    while len(selected_cands) < n + 1:
                        # X_rnd = draw_sobol_samples(bounds=bounds_cpu, n=10000, q=q, seed=seed).cpu().detach().numpy()
                        X_rnd = sobol.draw(10000).to(dtype=torch.float64).cpu().detach().numpy()
                        X_rnd = (kwargs['problem'].bounds[1].cpu().data.numpy() - kwargs['problem'].bounds[0].cpu().data.numpy()) \
                                * X_rnd + kwargs['problem'].bounds[0].cpu().data.numpy()
                        for node in kwargs['lamoo_boundary']:
                            boundary = node[0].classifier.svm
                            if len(X_rnd) == 0:
                                break
                            X_rnd = X_rnd[boundary.predict(X_rnd) == node[1]]  # node[1] store the direction to go

                        selected_cands = np.append(selected_cands, X_rnd, axis=0)
                        kwargs['lamoo_boundary'] = kwargs['lamoo_boundary'][:-1]

                    selected_cands = selected_cands[1:]
                    final_cands_idx = np.random.choice(len(selected_cands), n)
                    selected_cands = selected_cands[final_cands_idx]
                    X_rnd = torch.tensor(selected_cands, requires_grad=True)
                    X_rnd = X_rnd.reshape(X_rnd.shape[0], -1, kwargs['problem'].dim)
                    # print('lamoo init is finished!!!')
